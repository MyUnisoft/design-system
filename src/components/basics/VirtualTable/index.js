...
import { MultiGrid, Grid, ScrollSync, AutoSizer, CellMeasurerCache, CellMeasurer } from 'react-virtualized';
...
const VirtualTable = React.memo(({
  ...
  const cache = new CellMeasurerCache({ fixedWidth: true, defaultHeight: rowHeight, fixedHeight: true });
  ...
  const tableKeyName = useMemo(() => _tableKeyName || generateId(), [_tableKeyName]);
  const data = useMemo(() => order ? _orderBy(rowData, order, orderDirection) : rowData, [rowData, order, orderDirection]);
  ...
  const {
    selectedRows, dataRows, addRows, groupCollapsed
  } = context;

  useEffect(() => {
    if (defaultSelectedRows) send('selectRows', { selectedRows: defaultSelectedRows });
  ...
  const allSelectableKey = useMemo(() => new Set(
    allRows
      .filter(row => {
        const checkIfSocietyIsOpen = !(openSocieties.map(society => society?.id).includes(row?.company_id));
        const isSecuredRow = hasSecureSociety && row?.secured && checkIfSocietyIsOpen;

        return row.rowType !== ROW_TYPE.GROUP_BUTTON && row.rowType !== ROW_TYPE.ADD_BUTTON && !isSecuredRow; })
  ...
  const searchableColumns = useMemo(() => columns.filter((c) => !c.typeCell || allowedTypes.includes(c.typeCell)), [columns]);
  ...
  const displayableRows = useMemo(() =>  allRows
    .filter(row => (row.rowType === ROW_TYPE.GROUP_BUTTON || !groupCollapsed.has(row.groupId)) && matchFilter(searchableColumns, row, searchFilters.filters, searchFilters.q)),
  ...
  const calculateHeightFullTable = (_height) => {
    ...
    if (hasSelectionManager) { heightCalculate += 50; }
    if (hasFooter) { heightCalculate += 32; }

    return heightCalculate;
  };

  const size = useMemo(
    () => ({ height: calculateHeightFullTable(displayableRows.length * rowHeight), maxHeight: maxHeight, width: widthTable }),
    ...
    const render = () => (
      <div
        className={cx(
          styles.cell,
          {
            [styles.cell_alignRight]: [TYPE_CELL.AMOUNT].includes(typeCell) || alignText === ALIGN_CELL.RIGHT,
            [styles.cell_alignLeft]: alignText === ALIGN_CELL.LEFT,
            [styles.center] : [TYPE_CELL.CHECKBOX].includes(typeCell) || alignCenter,
      ...
      >
        {selectAll && (<div className={styles.cbCell}>
          <Checkbox
            name={`${header}_${columnIndex}`}
            id={`${header}_${columnIndex}`}
            checked={isAllCheckedColumn(keyColumn, displayableRows)}
            onChange={(e) => {
              send('selectAllColumn', { value: e.target.checked, keyColumn, changeOtherColumnOnClick });
            }
            }
            variant={variant}
          />
        </div>)}
        <div className={cx(styles.headerCell, wrapHeaderText && styles.headerCell_wrap, withBorders && styles.headerCell_border)}>
  ...
  const renderCell = ({ displayableRow, typeCell, keyColumn, columnIndex, optionList, renderComponent }) => {
    ...
    const _optionList = typeof optionList === OPERAND.FUNCTION
  ...
  const _renderBodyCell = useCallback(({
    ...
    const { [selectableRowKey]: selector, rowType } = displayableRows[rowIndex];
    ...
    const checkIfSocietyIsOpen = !(openSocieties.map(society => society?.id).includes(displayableRows[rowIndex]?.company_id));
    const isSecuredRow = hasSecureSociety && displayableRows[rowIndex]?.secured && checkIfSocietyIsOpen;
    ...
    const cell = (
      <div
        className={cx(styles.cell,
          ((rowIndex % 2) ? styles.odd : styles.even),
          {
            ...
            [styles.onError]: (hasError || (tooltipMessage && [TABLE_STATE.EDIT, TABLE_STATE.PERMANENT_EDIT].includes(state))) && !isGroupButton && !isAddButton,
            [styles.cell_withGrid]: withBorders,
            [styles.cell_alignLeft]: alignText === ALIGN_CELL.LEFT,
            [styles.cell_alignCenter]: alignText === ALIGN_CELL.CENTER,
            [styles.cell_alignRight]: alignText === ALIGN_CELL.RIGHT,
            [styles.cell_align]: [ALIGN_CELL.CENTER, ALIGN_CELL.BOTTOM,ALIGN_CELL.TOP].includes(alignVertical),
            [styles.cell_alignVerticalCenter]: alignVertical === ALIGN_CELL.CENTER,
            [styles.cell_alignVerticalBottom]: alignVertical === ALIGN_CELL.BOTTOM,
            [styles.cell_alignVerticalTop]: alignVertical === ALIGN_CELL.TOP,
            [styles.lockRow]: isSecuredRow,
            [styles.pointer]: !!simpleClick,
            [styles.cell_thumbnail]: [TYPE_CELL.THUMBNAIL].includes(typeCell)
          })}
        key={key}
        style={style}
      ...
      >
        {hasContextMenu ? (
          ...
          >
            {securedRender(renderCell({ displayableRow: displayableRows[rowIndex], typeCell, keyColumn, columnIndex, optionList, renderComponent }))}
          </ContextMenu>
        ) : securedRender(renderCell({ displayableRow: displayableRows[rowIndex], typeCell, keyColumn, columnIndex, optionList, renderComponent }))}
    ...
    function renderIfTooltip() {
      if (tooltipMessage && [TABLE_STATE.EDIT, TABLE_STATE.PERMANENT_EDIT].includes(state)) {
        return <ErrorTooltip title={tooltipMessage}>{cell}</ErrorTooltip>;
      }

      if (tooltipDetail) {
        return <Tooltip title={tooltipDetail} placement="top">{cell}</Tooltip>;
      }
  ...
  const isAllChecked = useMemo(
    () => _isEqual(allSelectableKey, selectedRows), [allSelectableKey, selectedRows]
  );
  ...
  const _renderHeaderCheckBoxCell = useCallback(({
    ...
    return (
      <div className={cx(styles.cell, styles.cbCell)} key={key} style={style}>
        {!selectableOnlyOneRow && (
          <div className={styles.checkbox}>
            <label htmlFor={`${rowIndex}_selectable_header_${tableKeyName}`} className={styles.container}>
              <input
                type="checkbox"
                ...
                onChange={(e) => {
                  send('toggleAllRow', { selectedRows: (e.target.checked) ? allSelectableKey : [] });
                }}
              />
  ...
  const _renderFooterCell = useCallback(({
    ...
    const { typeFooter, tooltipFooter, typeCell, footerValue, key: keyValue, alignText } = columns[columnIndex];

    const _getValue = () => {
      switch(typeFooter) {
      case TYPE_FOOTER_CELL.SUM:
        return displayableRows.reduce((accumulator, currentValue) => accumulator + (currentValue[keyValue] ? currentValue[keyValue] : 0), 0);
      case TYPE_FOOTER_CELL.AMOUNT:
        return formatNumber(Number(displayableRows.reduce((accumulator, currentValue) => accumulator + (currentValue[keyValue] ? currentValue[keyValue] : 0), 0)).toFixed(2));
      }
    };
    ...
    const _render = () => (
      ...
      >
        <div
          className={cx(styles.footerCell)}
        >
  ...
  const _renderCheckBoxCell = useCallback(({
    // eslint-disable-next-line react/prop-types
    key, parent, rowIndex, style
  }) => {
    const {
      groupId, rowType = ROW_TYPE.VIEW
    } = displayableRows[rowIndex];
    const isSelected = selectedRows.has(displayableRows[rowIndex][selectableRowKey]);
    ...
    function renderCheckboxCell() {
      ...
      return (
        <div className={styles.checkbox}>
          <label htmlFor={`${rowIndex}_selectable_${tableKeyName}`} className={styles.container}>
            <input
              type="checkbox"
              id={`${rowIndex}_selectable_${tableKeyName}`}
              name={`${rowIndex}_selectable_${tableKeyName}`}
              checked={isSelected}
              disabled={!can('selectItem') || !can('unselectItem') || isSecuredRow}
              onChange={(e) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {isGroupButton ? (<FontIcon name={isGroupCollapsed ? 'icon-plus1' : 'icon-minus'} size="16px" onClick={() => send(isGroupCollapsed ? 'expandGroup' : 'collapseGroup', { groupId })} />)
            : (rowType !== ROW_TYPE.NEW)
              ? renderCheckboxCell()
              : <FontIcon name="icon-trash" color="red" size="18px" onClick={() => send('delete', { id: displayableRows[rowIndex]?.id })} />
          }
        </div>
      </CellMeasurer>
    );
  ...
  const buttons = [...customButtons?.map(customButton => {
  ...
  if (['IDLE'].includes(state)) {
    ...
    if (modeDelete) {
      buttons.push({
        ...
        onClick: () => { handleRowsToDelete(selectedRows); }
      });
    }
  }
  if ([TABLE_STATE.ADD, TABLE_STATE.EDIT, TABLE_STATE.PERMANENT_EDIT].includes(state)) {
    buttons.push({
      _type: 'icon',
      iconName: 'icon-save',
      iconColor: 'white',
      ...
      disabled: !can('validate') || allRows.filter(e => [ROW_TYPE.NEW, ROW_TYPE.EDIT].includes(e.rowType)).length === 0,
      onClick: () => {
        switch (state) {
        case TABLE_STATE.ADD:
          handleValidateAdd(allRows.filter(e => e.rowType === ROW_TYPE.NEW));
          break;
        case TABLE_STATE.EDIT:
        case TABLE_STATE.PERMANENT_EDIT:
          handleValidateEdit(allRows.filter(e => e.rowType === ROW_TYPE.EDIT));
          break;
        }
        send('validate');
      }
  ...
  const totalColumnsWidth = useMemo(() => columns
    .reduce((sum, item) => sum
          + (Number.isNaN(Number(item?.width)) ? 0 : item.width), 0), [columns]);

  const totalColumnsSize = useMemo(() => columns
    .reduce((sum, item) => sum
          + (Number.isNaN(Number(item?.size)) ? 0 : item.size), 0), [columns]);

  const calculateHeightGridBody = (_height) => {
    let heightCalculate = _height - rowHeight;
    if (customSelectionHeader || hasSelectionManager) { heightCalculate -= 50; }
    if (hasFooter) { heightCalculate -= 32; }
  ...
  return (
    <div>
      { customSelectionHeader || hasSelectionManager
        && (
          <div className={styles.selectionManager}>
            ...
            <div>
              {(selectableRow || selectableOnlyOneRow) && I18n.t('common.selectedCount', { count: selectedRows.size })}
            </div>
            <div>
              ...
              >
                {actionsButton.map(({ label, onClick }, idx) => (
                  <MenuItem key={`${label}_${idx}`} onClick={() => { onClick(displayableRows); handleClose(); }}>
                    {label}
                  </MenuItem>
                ))}
              </Menu>
        ...
        <ScrollSync>
          {({
            ...
            <AutoSizer onResize={handleResize} style={size}>
              {({ height, width }) => {
                if (dynamicHeight && displayableRows.length * rowHeight <= calculateHeightGridBody(height)) {
                ...
                const _getColumnWidth = ({ index }) => {
                  const { width: widthColumn = -1, size = -1 } = columns[index];
                  if (size !== -1 && width > totalColumnsWidth) {
                    const averageColumn = (width - totalColumnsWidth - 20
                            - ((selectableRow || selectableOnlyOneRow) ? 30 : 0))
                        / totalColumnsSize;
                    const widthBySize = averageColumn * size;
                    return (widthBySize > minCellWidth) ? widthBySize : minCellWidth;
                  }
                  return ((widthColumn !== -1) ? widthColumn : minCellWidth) + spaceBetween;
                };

                return (
                  <div className={styles.GridRow}>
                    { (selectableRow || selectableOnlyOneRow || modeAdd || modeGroup) && (
                      <div
                      ...
                      >
                        ...
                        {customHeader && (
                          <Grid
                            ...
                            width={30}
                            height={customHeaderRef.current?.offsetHeight || 32}
                            rowHeight={customHeaderRef.current?.offsetHeight || 32}
                            columnWidth={30}
                            rowCount={1}
                            columnCount={1}
                          />
                        ...
                        <Grid
                          ...
                          columnWidth={30}
                          columnCount={1}
                          className={cx(styles.LeftSideGrid, styles.defaultStyle)}
                          height={height}
                    ...
                    <div className={styles.GridColumn}>
                      {customHeader && React.cloneElement(customHeader, { ref: customHeaderRef })}
                      {hasHeader && (
                        <div className={styles.gridContainer}>
                          <MultiGrid
                            ref={refHeaderGrid}
                            classNameBottomRightGrid={cx(styles.HeaderGrid, styles.defaultStyle, {
                            ...
                            width={width - ((selectableRow || selectableOnlyOneRow) ? 30 : 0)}
                          />
                        </div>
                      )}
                      <div className={styles.gridContainer}>
                        <MultiGrid
                          ref={refBodyGrid}
                          classNameBottomRightGrid={cx(styles.BodyGrid, styles.defaultStyle, {
                    ...
                    {hasFooter && <div className={styles.footer_wrap}>
                      {modeEdit && <div className={styles.footer_fakeCheckboxCell}/>}
                      <Grid
                        ref={refFooterGrid}
                        className={cx(styles.HeaderGrid, styles.defaultStyle)}
                        columnWidth={_getColumnWidth}
                        columnCount={columns.length}
                        height={32}
                        overscanColumnCount={0}
